...
public class GlobalErrorHandler extends AbstractErrorWebExceptionHandler {
  ...
  private Mono<ServerResponse> renderRuntimeExceptions(Throwable throwable) {
    ...
    if (throwable instanceof RecordNotFoundException) {
      return buildStandardResponse(HttpStatus.NOT_FOUND, RECORD_NOT_FOUND_CODE.getCode(), throwable.getMessage());
    }

    if (throwable instanceof ResourceAlreadyExistException) {
      return buildStandardResponse(HttpStatus.CONFLICT, RECORD_NOT_FOUND_CODE.getCode(), throwable.getMessage());
    }

    if (throwable instanceof ServerWebInputException) {
      return buildStandardResponse(HttpStatus.BAD_REQUEST, EMPTY_REQUIRED_FIELD_CODE.getCode(), INVALID_JSON_BODY_MESSAGE);
    }

    if (throwable instanceof ResponseStatusException) {
      return buildStandardResponse(HttpStatus.NOT_FOUND, RECORD_NOT_FOUND_CODE.getCode(), INVALID_ENDPOINT_MESSAGE);
    }
  ...
  private Mono<ServerResponse> buildStandardResponse(HttpStatus httpStatus, String code, String message) {

    logger.info("{}: '{}'", httpStatus, message);
    final ErrorResponseDTO errorResponseDTO = new ErrorResponseDTO(code, message);
  ...
  private Mono<ServerResponse> renderValidationException(ValidationException throwable) {
    ...
    final Function<ObjectError, String> buildParamErrorMsg = objectError -> ((FieldError) objectError).getField() + ": " + objectError.getDefaultMessage();

    final List<ErrorResponseDTO> errorResponseDTOList = throwable.getErrors()
            .stream()
            .map(objectError -> new ErrorResponseDTO(objectError.getCode(), buildParamErrorMsg.apply(objectError)))
  ...
  private Mono<ServerResponse> buildServerResponseWithErrorDTO(HttpStatus httpStatus, List<ErrorResponseDTO> errorResponseDTOList) {
